"""Fresh concurrent 11c on one80GBH100 with explicit answer finalization on official FP8.
...
GPU = 'H100'
app = modal.App('germanwiki-hf-fp8-concurrent-11c')
...
def validate_fresh(run_id, inputs, question_count=2):
    ...
    if 'fp8' not in run_id.lower() or not run_id.endswith('-concurrent-11c'):
        raise ValueError('Fresh run ID must contain fp8 and end in -concurrent-11c')
    ...
    if inputs.get("access_manifest", {}).get("schema") != SCHEMA:
        raise ValueError("11c requires the audited complementary group partition")
    if inputs.get("source_access_policy") != DISCOVERY_ACCESS_POLICY:
        raise ValueError("11c preserves discovery-only foreign GET access")
    if inputs.get('answer_final_reserve_tokens') != 512:
        raise ValueError('11c requires answer-only final reserve512')
    if inputs.get('coordinator_readiness_policy') != 'cold-start-600-v2':
        raise ValueError('11c requires cold-start-600-v2')
    if inputs.get('agent_log_policy') != 'no-agent-history-v1':
        raise ValueError('11c requires no agent-visible request history')
...
def execute(run_id, inputs, download_model=False, question_count=2):
    ...
    provenance = {'backend': 'Modal', 'model': metadata, 'resources': resources(settings),
                  ...
                  'experiment_variant': '11c-no-agent-history-v1',
                  ...
                  'migration': 'Exact11b3 questions/partition/budgets/cold600/final512; no agent-visible own/foreign request history or injection; host audit and notebooks preserved'}
...
def main(argv=None):
    ...
    inputs = {'records': [json.loads(line) for line in args.dataset.read_text().splitlines() if line.strip()],
              ...
              'agent_log_policy':'no-agent-history-v1',
