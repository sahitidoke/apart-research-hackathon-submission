"""Fresh concurrent 10c reference on one80GBH100 with explicit answer finalization on official FP8.
...
GPU = 'H100'
app = modal.App('germanwiki-hf-fp8-concurrent-10c')
...
def resources(settings):
    return {'gpu':GPU,'gpu_count':1,'gpu_memory_gb':80,'model_processes':1,'weight_copies':1,'max_num_seqs':2,'concurrent_agent_requests':2,'host_memory_mib':65536,'context_length':65536,'hard_job_seconds':JOB_SECONDS,
            'coordinator_readiness':settings['coordinator_readiness'],
...
def validate_fresh(run_id, inputs, question_count=2):
    ...
    if 'fp8' not in run_id.lower() or not run_id.endswith('-concurrent-10c'):
        raise ValueError('Fresh run ID must contain fp8 and end in -concurrent-10c')
    ...
    if manifest.get("schema") != SCHEMA or manifest.get("partition_audit", {}).get("schema") != VERSION:
        raise ValueError("10c requires the audited complementary 11b partition")
    if inputs.get("source_access_policy") != DISCOVERY_ACCESS_POLICY:
        raise ValueError("10c requires discovery-only source access")
    if inputs.get("answer_final_reserve_tokens") != 512:
        raise ValueError("10c fixes-v2 requires answer-only final reserve512")
...
def execute(run_id, inputs, download_model=False, question_count=2):
    ...
    provenance = {'backend': 'Modal', 'model': metadata, 'resources': resources(settings),
                  ...
                  'experiment_variant':'10c-complementary-research-reflection-fixes-v2',
                  ...
                  'migration': 'Reuses concurrent10b startup-v2 schedule with worker-owned SQLite cleanup and answer-only512 reserve inside2048 with exact11b complementary partition and two MLB questions; explicit finalization remains within budget; not a single-variable comparison with10b'}
...
def main(argv=None):
    ...
    inputs = {'records': [json.loads(line) for line in args.dataset.read_text().splitlines() if line.strip()],
              ...
              'answer_final_reserve_tokens':512, 'note_retry_policy': SHORT_NOTE_POLICY, 'no_peer_information': args.no_peer_information,
