"""Fresh concurrent 11b on one80GBH100 with explicit answer finalization on official FP8.
...
GPU = 'H100'
app = modal.App('germanwiki-hf-fp8-concurrent-11b')
...
def resources(settings):
    return {'gpu':GPU,'gpu_count':1,'gpu_memory_gb':80,'model_processes':1,'weight_copies':1,'max_num_seqs':2,'concurrent_agent_requests':2,'host_memory_mib':65536,'context_length':65536,'hard_job_seconds':JOB_SECONDS,
            ...
            'initial_research_tokens':2048,'research_stage_tokens':1536,'answer_tokens':2048,'reflection_tokens':1024,'answer_seconds':180,'answer_final_reserve_tokens':256,'answer_finalization_policy':'explicit-schema-final-only-v1',
...
def validate_fresh(run_id, inputs, question_count=2):
    ...
    if 'fp8' not in run_id.lower() or not run_id.endswith('-concurrent-11b'):
        raise ValueError('Fresh run ID must contain fp8 and end in -concurrent-11b')
    ...
    if inputs.get("access_manifest", {}).get("schema") != SCHEMA:
        raise ValueError("11b requires the audited complementary group partition")
    if inputs.get("source_access_policy") != DISCOVERY_ACCESS_POLICY:
        raise ValueError("11b preserves discovery-only foreign GET access")
...
def execute(run_id, inputs, download_model=False, question_count=2):
    ...
    provenance = {'backend': 'Modal', 'model': metadata, 'resources': resources(settings),
                  ...
                  'experiment_variant': '11b-complementary-document-families',
                  ...
                  'migration': 'Inherited concurrent11a phase protocol and explicit finalization; new11b engineered document-family discovery partition; one shared model onH100; preserved11a routes'}
