"""Fresh concurrent 11d on one80GBH100 with explicit answer finalization on official FP8.
...
from orchestrator.simulated_web.complementary_partition import SCHEMA
...
from orchestrator.simulated_web.concurrent_collaboration_pilot import build_settings, run_concurrent_collaboration_pilot

GPU = 'H100'
app = modal.App('germanwiki-hf-fp8-concurrent-11d')


def resources(settings):
    return {'coordinator_readiness':settings['coordinator_readiness'],'gpu':GPU,'gpu_count':1,'gpu_memory_gb':80,'model_processes':1,'weight_copies':1,'max_num_seqs':2,'concurrent_agent_requests':2,'host_memory_mib':65536,'context_length':65536,'hard_job_seconds':JOB_SECONDS,
            'phase_records':settings['maximum_phases'],'normal_model_phases':4+8*settings['question_count'],
            ...
            'publication_repairs':'at most512+1024tokens per research/reflection publication, already inside solver ceiling',
            'initial_research_tokens':2048,'research_stage_tokens':1536,'answer_tokens':2048,'reflection_tokens':1024,'answer_seconds':180,'answer_final_reserve_tokens':512,'answer_finalization_policy':'explicit-schema-final-only-v1',
            'source_call_caps':{'initial_research':8,'research1':4,'research2':4,'answer':4,'reflection':4},
...
def validate_fresh(run_id, inputs, question_count=2):
    ...
    if 'fp8' not in run_id.lower() or not run_id.endswith('-concurrent-11d'):
        raise ValueError('Fresh run ID must contain fp8 and end in -concurrent-11d')
    if inputs.get('inference_profile') != 'hf-fp8-v1':
        raise ValueError('HF FP8 inference profile required')
    if inputs.get("access_manifest", {}).get("schema") != SCHEMA:
        raise ValueError("11d requires the audited complementary group partition")
    if inputs.get("source_access_policy") != DISCOVERY_ACCESS_POLICY:
        raise ValueError("11d preserves discovery-only foreign GET access")
    if inputs.get('answer_final_reserve_tokens') != 512:
        raise ValueError('11d requires answer-only final reserve512')
    if inputs.get('coordinator_readiness_policy') != 'cold-start-600-v2':
        raise ValueError('11d requires cold-start-600-v2')
    if inputs.get('agent_log_policy') != 'no-agent-history-v1':
        raise ValueError('11d requires no agent-visible request history')
    if inputs.get('notebook_title_policy') != 'agent-first-line-v1':
        raise ValueError('11d requires agent first-line notebook titles')
    settings = build_settings(**inputs)[0]
    if inputs["access_manifest"]["partition_audit"]["selected_questions"] != settings["question_ids"]["agent-1"]:
        raise ValueError("Partition selected questions must match pilot questions")
...
def execute(run_id, inputs, download_model=False, question_count=2):
    ...
    provenance = {'backend': 'Modal', 'model': metadata, 'resources': resources(settings),
                  ...
                  'experiment_variant': '11d-agent-first-line-titles-v1',
                  'launch_source_sha256': {name: hashlib.sha256(Path(__file__).with_name(name).read_bytes()).hexdigest() for name in (Path(__file__).name, 'concurrent_collaboration_pilot.py')},
                  ...
                  'migration': 'Exact11c no-history controls/inputs/budgets; agent first nonblank line supplies120character notebook title; exact body/provenance preserved; search title affects existing BM25 ranking'}
    ...
    try:
        ...
        return run_concurrent_collaboration_pilot(destination, client, **inputs, provenance=provenance,
...
def main(argv=None):
    ...
    inputs = {'records': [json.loads(line) for line in args.dataset.read_text().splitlines() if line.strip()],
              ...
              'answer_final_reserve_tokens':512, 'coordinator_readiness_policy':'cold-start-600-v2',
              'agent_log_policy':'no-agent-history-v1','notebook_title_policy':'agent-first-line-v1',
