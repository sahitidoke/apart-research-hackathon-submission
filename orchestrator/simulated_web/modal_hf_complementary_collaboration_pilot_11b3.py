"""Fresh concurrent 11b3 on one80GBH100 with explicit answer finalization on official FP8.
...
GPU = 'H100'
app = modal.App('germanwiki-hf-fp8-concurrent-11b3')
...
def validate_fresh(run_id, inputs, question_count=2):
    ...
    if 'fp8' not in run_id.lower() or not run_id.endswith('-concurrent-11b3'):
        raise ValueError('Fresh run ID must contain fp8 and end in -concurrent-11b3')
    ...
    if inputs.get("access_manifest", {}).get("schema") != SCHEMA:
        raise ValueError("11b3 requires the audited complementary group partition")
    if inputs.get("source_access_policy") != DISCOVERY_ACCESS_POLICY:
        raise ValueError("11b3 preserves discovery-only foreign GET access")
    if inputs.get('answer_final_reserve_tokens') != 512:
        raise ValueError('11b3 requires answer-only final reserve512')
    if inputs.get('coordinator_readiness_policy') != 'cold-start-600-v2':
        raise ValueError('11b3 requires cold-start-600-v2')
...
def execute(run_id, inputs, download_model=False, question_count=2):
    ...
    provenance = {'backend': 'Modal', 'model': metadata, 'resources': resources(settings),
                  ...
                  'experiment_variant': '11b3-coordinator-cold-start-600-v2',
                  ...
                  'migration': 'Exact11b2 inputs/final512/cleanup; first coordinator startup600 only, later300; extra300 admission guard; historical startup300 preserved'}
