"""Fresh concurrent 11b2 on one80GBH100 with explicit answer finalization on official FP8.
...
GPU = 'H100'
app = modal.App('germanwiki-hf-fp8-concurrent-11b2')
...
def validate_fresh(run_id, inputs, question_count=2):
    ...
    if 'fp8' not in run_id.lower() or not run_id.endswith('-concurrent-11b2'):
        raise ValueError('Fresh run ID must contain fp8 and end in -concurrent-11b2')
    ...
    if inputs.get("access_manifest", {}).get("schema") != SCHEMA:
        raise ValueError("11b2 requires the audited complementary group partition")
    if inputs.get("source_access_policy") != DISCOVERY_ACCESS_POLICY:
        raise ValueError("11b2 preserves discovery-only foreign GET access")
    if inputs.get('answer_final_reserve_tokens') != 512:
        raise ValueError('11b2 requires answer-only final reserve512')
...
def execute(run_id, inputs, download_model=False, question_count=2):
    ...
    provenance = {'backend': 'Modal', 'model': metadata, 'resources': resources(settings),
                  ...
                  'experiment_variant': '11b2-answer-final512-worker-owned-cleanup',
                  ...
                  'migration': 'Exact11b complementary inputs; answer-only final reserve512 inside2048; worker-owned SQLite cleanup; cancellation initiator unknown; historical answer reserve256 preserved'}
...
def main(argv=None):
    ...
    inputs = {'records': [json.loads(line) for line in args.dataset.read_text().splitlines() if line.strip()],
              ...
              'answer_final_reserve_tokens':512,
