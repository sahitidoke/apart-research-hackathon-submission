"""Fresh 11a two-stage question research and exact publication on official FP8.
...
from orchestrator.simulated_web.collaboration_pilot import build_settings, run_collaboration_pilot

GPU = 'L40S'
app = modal.App('germanwiki-hf-fp8-collaboration-pilot-11a')


def resources(settings):
    return {'gpu':GPU,'gpu_count':1,'host_memory_mib':65536,'context_length':65536,'hard_job_seconds':JOB_SECONDS,
            ...
            'initial_research_tokens':2048,'research_stage_tokens':1536,'answer_tokens':2048,'reflection_tokens':1024,
...
def validate_fresh(run_id, inputs, question_count=2):
    ...
    if 'fp8' not in run_id.lower() or not run_id.endswith('-collaboration-pilot-11a'):
        raise ValueError('Fresh run ID must contain fp8 and end in -collaboration-pilot-11a')
...
def execute(run_id, inputs, download_model=False, question_count=2):
    ...
    provenance = {'backend': 'Modal', 'model': metadata, 'resources': resources(settings),
                  ...
                  'migration': 'New11a protocol and same-topic question sequence; checkpoint incompatible with10a; same corrected official FP8 transport; not an isolated schedule ablation'}
    ...
    try:
        ...
        return run_collaboration_pilot(destination, client, **inputs, provenance=provenance,
