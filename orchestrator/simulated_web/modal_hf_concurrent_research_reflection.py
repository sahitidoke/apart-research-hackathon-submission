"""Fresh concurrent 10b reference on one80GBH100 with explicit answer finalization on official FP8.
...
GPU = 'H100'
app = modal.App('germanwiki-hf-fp8-concurrent-10b')
...
def resources(settings):
    return {'gpu':GPU,'gpu_count':1,'gpu_memory_gb':80,'model_processes':1,'weight_copies':1,'max_num_seqs':2,'concurrent_agent_requests':2,'host_memory_mib':65536,'context_length':65536,'hard_job_seconds':JOB_SECONDS,
            ...
            'initial_research_tokens':8192,'answer_tokens':2048,'reflection_tokens':4096,'answer_seconds':180,'answer_final_reserve_tokens':256,'answer_finalization_policy':'explicit-schema-final-only-v1',
...
def validate_fresh(run_id, inputs, question_count=2):
    ...
    if 'fp8' not in run_id.lower() or not run_id.endswith('-concurrent-10b'):
        raise ValueError('Fresh run ID must contain fp8 and end in -concurrent-10b')
...
def execute(run_id, inputs, download_model=False, question_count=2):
    ...
    provenance = {'backend': 'Modal', 'model': metadata, 'resources': resources(settings),
                  ...
                  'migration': 'Concurrent10b research/reflection and explicit finalization opt-in; one shared model onH100; preserved serial10a/10b and11a launchers; no runtime-fit or speedup claim'}
