"""Fresh concurrent 11a on one80GBH100 with explicit answer finalization on official FP8.
...
GPU = 'H100'
app = modal.App('germanwiki-hf-fp8-concurrent-11a')
...
def validate_fresh(run_id, inputs, question_count=2):
    ...
    if 'fp8' not in run_id.lower() or not run_id.endswith('-concurrent-11a'):
        raise ValueError('Fresh run ID must contain fp8 and end in -concurrent-11a')
...
def execute(run_id, inputs, download_model=False, question_count=2):
    ...
    provenance = {'backend': 'Modal', 'model': metadata, 'resources': resources(settings),
                  ...
                  'migration': 'Concurrent11a and explicit finalization opt-in; one shared model onH100; preserved serial11a launcher; no runtime-fit or speedup claim'}
