"""Fresh 10a topic research and reset-per-question answer/reflection on official FP8.
...
from orchestrator.simulated_web.research_reflection import build_settings, run_research_reflection

GPU = 'L40S'
app = modal.App('germanwiki-hf-fp8-research-reflection')
...
def resources(settings):
    return {'gpu':GPU,'gpu_count':1,'host_memory_mib':65536,'context_length':65536,'hard_job_seconds':JOB_SECONDS,
            ...
            'reflection_repairs':'at most512+1024tokens per reflection, already inside solver ceiling',
            'initial_research_tokens':8192,'answer_tokens':2048,'reflection_tokens':4096,
...
def validate_fresh(run_id, inputs, question_count=6):
    if type(question_count) is not int or question_count not in (2,6):
        raise ValueError("Question count must be2(pilot) or6(reference)")
    validate_run_id(run_id)
    if 'fp8' not in run_id.lower() or not run_id.endswith('-research-reflection'):
        raise ValueError('Fresh run ID must contain fp8 and end in -research-reflection')
...
def execute(run_id, inputs, download_model=False, question_count=6):
    ...
    try:
        ...
        return run_research_reflection(destination, client, **inputs, provenance=provenance,
...
def main(argv=None):
    ...
    parser.add_argument('--question-count',type=int,choices=[2,6],default=6)
