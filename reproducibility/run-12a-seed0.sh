#!/usr/bin/env bash
set -euo pipefail
# 12a seed0: six-question reference; continue completed seed0 parent afterQ2.
# One H100, no history, descriptive titles;123904full-path maximumtokens. No automatic launch.
mode=(--validate-only)
run_id='germanwiki-6-012a-fp8-20260913-seed0-concurrent-12a'
run_id_set=false
...
exec uv run --frozen python -m orchestrator.simulated_web.modal_hf_reference_12a \
  ...
  --run-id "$run_id" --question-count 6 --preparation-safety-seconds 1200 --model-seed 0 --resume-checkpoint runs/downloads/10d2-20260913/checkpoints/rounds-002 --resume-run-id germanwiki-2-010e-fp8-20260913-h100-d2-prep1200-concurrent-10e --resume-round 2 \
