#!/usr/bin/env bash
set -euo pipefail
# 12d seed0: six-question reference; fresh initial research plus all six questions.
# One H100, no history, union source discovery;123904full-path maximumtokens. No automatic launch.
mode=(--validate-only)
run_id='germanwiki-6-012d-fp8-20260913-seed0-answer600-v3-concurrent-12d'
run_id_set=false
...
exec uv run --frozen python -m orchestrator.simulated_web.modal_hf_reference_12d \
  ...
  --run-id "$run_id" --question-count 6 --preparation-safety-seconds 1200 --model-seed 0 \
