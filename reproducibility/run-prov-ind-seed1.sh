#!/usr/bin/env bash
set -euo pipefail
# prov-ind seed 1: the second question draw. 30 questions (10 each at 2, 3, 4
# hops) x 6 conditions + 30 seeded planner episodes = 210 episodes. Greedy decoding.
# Billed GPU job (L40S, several hours). No automatic launch.
#
# At temperature 0 a seed does not resample: it redraws *which* questions are
# asked and *which* distractor is planted. So this is a second question sample,
# not a noise estimate -- see APPENDIX.md section I before comparing the two.
#
...
command=(modal run ${extra[@]+"${extra[@]}"} orchestrator/provenance/modal_run.py
         ...
         --max-new-tokens 1024 --seed 1 --paraphrase-mode message,sentence)
